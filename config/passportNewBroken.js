...
module.exports = function(passport) {
    ...
    passport.use('local-signup-username', new LocalStrategy({
        usernameField : 'username',
        passReqToCallback : true 
    },
    ...
    passport.use('local-signup-password', new LocalStrategy({
        passwordField : 'password',
        passReqToCallback : true 
    },
    function(req, password, done) {
        process.nextTick(function() {
            ...
            db.serialize(function() {
                db.all("SELECT * FROM local WHERE username = '"+user.name+"'", function(err, rows) {
                    if (err){
                        console.log(err)
                        return done(err)
                    }
                    if (rows !=0){
                        ...
                        db.all("INSERT INTO local (username, password) VALUES " +
                            "('"+user.name+"', '"+hashPassword(password)+"')", function(err,rows){});
                        var temp = {} ;
                        temp.name = user.name;
                        temp.id = user.name;
                        ...
                        fs.open("./views/userfiles/"+user.name+"/userFiles.db", "w");
                        var noteList = new sqlite3.Database("./views/userfiles/"+user.name+"/userFiles.db");
